...
export default function SectionDesratizacao({
  defaultDate,
  setValue,
  errors,
  control,
  ...
  return (
    <section className="w-full mt-8 border-solid border-2 border-astro-500 rounded-lg p-5">
      ...
      <div className="grid md:grid-cols-1 gap-4 mt-5">
        <label>
          <span className="text-gray-700 mb-3 text-md dark:text-astro-100 block">
            Descrição do serviço de Desratização
            {errors.dr_description && (
              <span className="ml-2 text-red-400 text-sm">
          ...
          <Controller
            ...
            render={({ field }) => (
              <InputTextarea
                ...
                className="w-full"
                onKeyPress={enterKeyPrevent}
                placeholder="Descreva o serviço de Desratização"
                autoResize
              />
            )}
          />
