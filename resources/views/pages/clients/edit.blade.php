...
<div class="flex flex-wrap mt-5" id="tabs-id">
    <div class="w-full">
        ...
        <li class="-mb-px mr-2 last:mr-0 flex-auto text-center">
            <a class="flex items-center justify-center text-sm font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal {{ $clientType == 0 ? "text-white bg-green-600" : "text-green-600 bg-white dark:bg-gray-800 dark:text-white cursor-not-allowed" }}">
        ...
        <li class="-mb-px mr-2 last:mr-0 flex-auto text-center">
            <a class="flex items-center justify-center text-sm font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal {{ $clientType == 1 ? "text-white bg-green-600" : "text-green-600 bg-white dark:bg-gray-800 dark:text-white cursor-not-allowed" }}">
            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <li class="-mb-px mr-2 last:mr-0 flex-auto text-center">
            <a href="#" class="flex items-center justify-center text-sm font-bold uppercase px-5 py-3 shadow-lg rounded block leading-normal text-green-600 bg-white dark:bg-gray-800 dark:text-white cursor-not-allowed">
                <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="tab-content tab-space">
                    <div class="toggle-content {{ $clientType == 0 ? "is-visible" : "" }} " id="tab-legal">
                        <div class="px-4 py-3 mb-8 mt-4 bg-white rounded-lg shadow-md dark:bg-gray-800 loading">
                            ...
                            <form id="ClientEditFormLegal" method="POST">
                                @csrf
                                @method('PUT')
                    ...
                    <div class="toggle-content {{ $clientType == 1 ? "is-visible" : "" }} " id="tab-individual">
                        <div class="px-4 py-3 mb-8 mt-4 bg-white rounded-lg shadow-md dark:bg-gray-800 loading">
                            ...
                            <form id="ClientEditFormIndividual" method="POST">
                                @csrf
                                @method('PUT')
                    ...
                    <div class="toggle-content" id="tab-subsidiary">
                        <div class="px-4 py-8 mb-8 mt-4 bg-white justify-self-center rounded-lg shadow-md dark:bg-gray-800 loading flex justify-center items-center">
...
<div class="fixed top-0 left-0 z-50 w-screen h-screen flex items-center justify-center hidden" style="background: rgba(0, 0, 0, 0.3);" id="complete-loader">
...
<script>
    ...
    // Send Forms
    $('#ClientEditFormLegal').submit(function (e) { 
        e.preventDefault();
        var data = $(this).serializeArray();
        sendDataToCreateClientViaAjax(data);
    });

    $('#ClientEditFormIndividual').submit(function (e) { 
        e.preventDefault();
        var data = $(this).serializeArray();
        sendDataToCreateClientViaAjax(data);
    });
    ...
    function sendDataToCreateClientViaAjax(data) {
        $.ajax({
            type: 'ajax',
            method: 'POST',
            url: "{{ route('clientes.update', ['cliente' => $client->id]) }}",
            data: data,
            dataType: 'json',
            statusCode: {
                201: handleSuccess
            ...
            error: function (xhr, status, error) {
                ...
                Swal.fire(
                    ...
                    'Não foi possível atualizar os dados esse cliente!' + '<br/>' +
